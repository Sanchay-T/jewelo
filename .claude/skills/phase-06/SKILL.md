---
name: phase-06
description: Implement only motion generation, evaluation, and player UX.
...
1. Read `CLAUDE.md`, `docs/goals/06-motion.md`, `docs/AGENT-WORKFLOW.md`, and `docs/VERIFICATION.md` completely.
2. Run `./scripts/doctor.sh`, `./scripts/verify-foundation.sh`, `git status --short --branch`, and inspect existing PRs/checks.
3. Confirm the work is on `phase/06-motion` based on `rebuild/v2-first-principles`. Use `./scripts/new-phase.sh 06` from the integration checkout when a worktree does not exist.
...
10. Stop. Do not merge and do not begin Phase 07.
