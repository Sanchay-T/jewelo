---
name: phase-02
description: Build only the responsive logged-in UX prototype with mocks.
...
1. Read `CLAUDE.md`, `docs/goals/02-ux-prototype.md`, `docs/AGENT-WORKFLOW.md`, and `docs/VERIFICATION.md` completely.
2. Run `./scripts/doctor.sh`, `./scripts/verify-foundation.sh`, `git status --short --branch`, and inspect existing PRs/checks.
3. Confirm the work is on `phase/02-ux-prototype` based on `rebuild/v2-first-principles`. Use `./scripts/new-phase.sh 02` from the integration checkout when a worktree does not exist.
...
10. Stop. Do not merge and do not begin Phase 03.
