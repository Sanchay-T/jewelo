---
name: phase-07
description: Implement only estimates, quotes, orders, and operator workflow.
...
1. Read `CLAUDE.md`, `docs/goals/07-commerce.md`, `docs/AGENT-WORKFLOW.md`, and `docs/VERIFICATION.md` completely.
2. Run `./scripts/doctor.sh`, `./scripts/verify-foundation.sh`, `git status --short --branch`, and inspect existing PRs/checks.
3. Confirm the work is on `phase/07-commerce` based on `rebuild/v2-first-principles`. Use `./scripts/new-phase.sh 07` from the integration checkout when a worktree does not exist.
...
10. Stop. Do not merge and do not begin Phase 08.
