---
name: phase-00
description: Research and finalize architecture without implementing the application.
...
1. Read `CLAUDE.md`, `docs/goals/00-research-architecture.md`, `docs/AGENT-WORKFLOW.md`, and `docs/VERIFICATION.md` completely.
2. Run `./scripts/doctor.sh`, `./scripts/verify-foundation.sh`, `git status --short --branch`, and inspect existing PRs/checks.
3. Confirm the work is on `phase/00-research-architecture` based on `rebuild/v2-first-principles`. Use `./scripts/new-phase.sh 00` from the integration checkout when a worktree does not exist.
...
10. Stop. Do not merge and do not begin Phase 01.
