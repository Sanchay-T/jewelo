---
name: phase-01
description: Build only the reproducible production repository foundation.
...
1. Read `CLAUDE.md`, `docs/goals/01-repo-foundation.md`, `docs/AGENT-WORKFLOW.md`, and `docs/VERIFICATION.md` completely.
2. Run `./scripts/doctor.sh`, `./scripts/verify-foundation.sh`, `git status --short --branch`, and inspect existing PRs/checks.
3. Confirm the work is on `phase/01-repo-foundation` based on `rebuild/v2-first-principles`. Use `./scripts/new-phase.sh 01` from the integration checkout when a worktree does not exist.
...
10. Stop. Do not merge and do not begin Phase 02.
