---
name: phase-03
description: Implement only domain, persistence, auth boundary, APIs, and realtime.
...
1. Read `CLAUDE.md`, `docs/goals/03-domain-data-realtime.md`, `docs/AGENT-WORKFLOW.md`, and `docs/VERIFICATION.md` completely.
2. Run `./scripts/doctor.sh`, `./scripts/verify-foundation.sh`, `git status --short --branch`, and inspect existing PRs/checks.
3. Confirm the work is on `phase/03-domain-data-realtime` based on `rebuild/v2-first-principles`. Use `./scripts/new-phase.sh 03` from the integration checkout when a worktree does not exist.
...
10. Stop. Do not merge and do not begin Phase 04.
