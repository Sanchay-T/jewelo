-- SIMPLE-1 (25 Sep 2026). There is no stencil, no identity anchor and no
-- verification in the image path any more: the shopper approves a name and a
-- look, one prompt string goes to the image model, and the photograph it
-- returns is the piece. So the completion authority no longer restates a
-- verdict nobody produces.
--
-- Removed from 20260924000000_completion_requires_single_connected_piece.sql:
--   * the identity_artifacts lineage lookup ('completion identity mismatch')
--   * the whole verification_result verdict block ('completion verification
--     missing'), including the nameCheck condition
--
-- Everything else is byte-identical: the cancellation fence, the lineage and
-- checkpoint comparisons, the idempotent asset insert, the attempt
-- reconciliation, the ready transition, the dependent release and the audit
-- event. The NOT NULL columns are still written - the worker fills
-- identity_fingerprint from the revision's own anchor and verification_result
-- with '{}'::jsonb - so no column is dropped and no row is rewritten.
