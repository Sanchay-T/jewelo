...
- Staging: `https://jewelo-staging-gqumd.ondigitalocean.app`, DigitalOcean app `ec09c9fd-84e4-45c5-b60a-fd62277af322`, region blr. It runs `provider: real` (`providerMode()` forces real when `NODE_ENV=production`; `deploy.sh` strips `PROVIDER_MODE`). Spend is bounded by `runtime_policy`, not by mock: the row set on 23 Sep 2026 caps one shopper at 4 runs and USD 8 reserved per day and the whole shop at 16 runs and USD 20 reserved per day, with 3 paid attempts per task, so staging can and does spend inside those caps; never raise them without Sanchay. Locally `PROVIDER_MODE=mock` still works.
