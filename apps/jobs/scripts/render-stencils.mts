 ...
 *
 * D-023 made the lettering a property of the construction, so the plane has to
 * be drawn in the same letterform as the piece under test or the comparison
 * would be between two different names. `origami-ribbon` is the letters-alone
 * construction of the boxy group (its folded facets are a finish the stencil
 * never claims) and `classical` is the letters-alone construction of the
 * customer's own lettering, so each group has a plane with its own letterform
 * and neither plane carries a frame or a rail.
 */
const PRE_RING_CONSTRUCTION = CONSTRUCTION_LETTERING[specificationConstruction]
  ? "origami-ribbon"
  : "classical";
 ...
 */
const PRE_RING_LETTERING_MATCHES = (script: IdentityScript): boolean =>
  JSON.stringify(CONSTRUCTION_LETTERING[specificationConstruction]?.[script]) ===
  JSON.stringify(CONSTRUCTION_LETTERING[PRE_RING_CONSTRUCTION]?.[script]);
...
for (const name of NAMES) {
  for (const script of SCRIPTS) {
    for (const lettering of LETTERINGS) {
      ...
      // A lab cell that the engine refuses is a result, not a crash: the post
      // cap and the pinhole floor both refuse real names, and a sweep that dies
      // on the first one measures nothing. The cell is printed and skipped, and
      // the four tables below are over the cells that rendered.
      let rendered: Awaited<ReturnType<typeof renderIdentityAnchor>>;
      try {
        ...
        console.log(`LAB ${file.padEnd(34)}refused ${(error as Error).message}`);
        continue;
      }
      ...
      // D-023: the piece is drawn in the lettering its construction imposes,
      // so the independent shaping is of the *drawn* text at the same weight
      // and tracking. Shaping the approved text at the default instance would
      // measure a name this stencil does not carry.
      const shaped = await shapeText({
        fontBytes: new Uint8Array(readFileSync(identityFontUrl(fontFile))),
        text: rendered.report.drawnText,
        script,
        weight: rendered.report.claimed.letteringWeight || undefined,
        trackingUnits: rendered.report.claimed.letteringTrackingUnits,
      });
      ...
      try {
        ...
        console.log(
          `LAB ${file.padEnd(34)}refused plane:${(error as Error).message}`,
        );
        continue;
      }
      ...
      const carrierPixelDelta = sweepCarrier
        ? PRE_RING_LETTERING_MATCHES(script)
...
for (const name of MATRIX_NAMES) {
  for (const script of SCRIPTS) {
    for (const style of LIVE_IDENTITY_STYLES) {
      ...
      // D-023: the letters-only plane is drawn in the same lettering as the
      // piece under test, so it can refuse for the same reasons the piece can -
      // a boxy name tightened until a counter is squeezed under the pinhole
      // floor is the measured case. That is a cell this harness cannot measure,
      // not a reason to abandon the other 575, so it is recorded like any other
      // refusal and the sweep goes on.
      let pre: PreRingMask;
      try {
        pre = await renderPreRingMask(
          text,
        ...
        const refused = `plane:${(error as Error).message}`;
        matrix.set(cellKey(name.label, script, style), {
          file, refused, carrierDelta: -1, carrierWelds: 0, carrierBridges: 0,
          carrierNameScale: 1, components: 0, jumpRings: 0, ringPlacement: "-",
          ringCarriers: [], ringHoleFloor: 0, ringHolesFound: 0,
          ringHolesAbove: 0, ringHoleSizes: [], holesAboveGlyphTop: 0,
          measuredPunchedByRings: 0, measuredWeldedIntoRingMetal: 0,
          claimedPunchedByRings: 0, claimedWeldedIntoRingMetal: 0,
          foreignUnderFillet: 0, ringSpan: 0, ringSpanRatio: 0, ringTilt: 0,
          ringOverhang: 0, pinholes: 0, seatSearchSteps: 0, ringLift: 0,
          ringPost: 0, anchorDepth: 0, ringCentres: [],
          islandsBeforeBridging: 0, bridges: 0, moved: 0, recentreScale: 1,
        });
        console.log(`CELL ${file.padEnd(34)}refused ${refused}`);
        continue;
      }
...
if (sweepCarrier) {
  ...
  console.log(
    `MATRIX CARRIER-DELTA ${carrierOff.length}/${carrierCells.length} ${specificationConstruction} cells where this harness and the engine disagree on the structure` +
      (carrierCells.length < matrixCells.length
        ? ` (${matrixCells.length - carrierCells.length} cells not measurable: the letters-only plane is drawn in a different lettering from the piece, D-023)`
        : "") +
      (carrierOff.length
        ? `: ${carrierOff
            .slice(0, 12)
  ...
  // The weld count and the name scale are read off the piece itself, not off
  // the letters-only plane, so they stay over every cell of the sweep even
  // where `CARRIER-DELTA` could not be measured.
  const unheld = matrixCells.filter(
    (cell) => cell.carrierWelds < 2 || cell.carrierBridges > 0,
  );
  console.log(
    `MATRIX CARRIER-WELDS ${matrixCells.length - unheld.length}/${matrixCells.length} cells hold the name into the structure with at least two welds and no connector bar after them` +
      (unheld.length
        ? `: ${unheld
            .slice(0, 12)
            .map(
