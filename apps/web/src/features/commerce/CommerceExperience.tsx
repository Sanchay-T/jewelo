...
const fulfillmentStages: Array<{
...
}> = [
  {
    ...
    detail: "Accepted quote and design snapshot recorded",
  },
  {
    id: "in-production",
    label: "In production",
    detail: "Atelier work has started",
  },
  {
    id: "quality-check",
    label: "Quality check",
    detail: "Final specification review",
  },
  ...
  return (
    <div className={styles.detailStack}>
      <p className={styles.price}>
        {formatMoney(estimate.low, locale)}–{formatMoney(estimate.high, locale)}
      </p>
      <div className={styles.metaRow}>
        <span className={styles.confidence}>Medium confidence</span>
        <span>
          Gold snapshot {formatDate(estimate.goldPriceTimestamp, locale, true)}
        </span>
      </div>
      <div>
      ...
      <p className={styles.validity}>
        Indicative until {formatDate(estimate.expiresAt, locale)}. Final price
        depends on confirmed weight, stones, and atelier review.
      </p>
    </div>
  );
}
...
function Snapshot({
  ...
  const revisionId =
    order?.revisionId ?? quote?.snapshot.revisionId ?? estimate?.revisionId;
  const directionId =
    order?.directionId ??
    quote?.snapshot.directionId ??
    estimate?.directionId ??
    design.selectedDirectionId;

  return (
    <section
      className={styles.snapshot}
      aria-labelledby="commercial-snapshot-heading"
    >
      <div>
      ...
      <dl className={styles.snapshotGrid}>
        <div>
          ...
          <dd>{revisionId ?? "Not captured"}</dd>
        </div>
        <div>
          <dt>Selected direction</dt>
          <dd>{directionId ?? "Not selected"}</dd>
        </div>
        <div>
          <dt>Estimate confidence</dt>
          <dd>{estimate?.confidence ?? "Not calculated"}</dd>
        </div>
        <div>
          <dt>Quote state</dt>
          <dd>{quote?.status ?? "Not requested"}</dd>
        </div>
        {order ? (
          <div>
            <dt>Accepted total</dt>
            <dd>{formatMoney(order.acceptedTotal, locale)}</dd>
          </div>
        ) : null}
        {order ? (
          <div>
            <dt>Accepted on</dt>
            <dd>{formatDate(order.acceptedAt, locale, true)}</dd>
          </div>
        ) : null}
      </dl>
      <p className={styles.disclaimer}>
        Fixture values are fictional. This page is not an offer, payment
        request, or manufacturing promise.
      </p>
    </section>
  );
}
    ...
    <main className={styles.page} dir={locale === "ar" ? "rtl" : "ltr"}>
      ...
      <header className={styles.hero}>
        <div>
          ...
          <p>
            Each step is separate and auditable. Your accepted quote keeps the
            exact revision, direction, assumptions, and price snapshot it was
            based on.
          </p>
        </div>
      ...
      <div className={styles.mainGrid}>
        <section className={styles.card} aria-labelledby="estimate-heading">
          <div className={styles.cardHeading}>
            <div>
              ...
              <h2 id="estimate-heading">Estimate and quote</h2>
            </div>
          ...
          {!estimate ? (
            <div className={styles.emptyState}>
              ...
              <p>
                The fixture uses the selected immutable revision and direction.
                It is not a final jeweler quote.
              </p>
              <button
                ...
                onClick={() =>
                  runAction(
                    "estimate",
                    "Estimate calculated and assumptions captured.",
              ...
              {!design.selectedDirectionId ? (
                <p className={styles.hint}>
                  Select a ready direction in the studio first.
                </p>
              ) : null}
            </div>
          ) : (
          ...
          {estimate && !quote ? (
            <button
              ...
              onClick={() =>
                runAction(
                  "request",
                  "Quote request sent for operator review.",
          ...
          {quote?.status === "requested" ? (
            <div className={styles.notice} role="status">
              ...
              <div>
                ...
                <p>
                  An operator must review the fixture assumptions before a quote
                  is issued.
                </p>
              </div>
            </div>
          ...
          {quote?.status === "issued" ? (
            <div className={styles.quotePanel}>
              ...
              <p>
                Valid until {formatDate(quote.expiresAt, locale)}. Acceptance
                records this quote and its estimate snapshot; no payment is
                collected.
              </p>
              <button
                className={styles.primaryButton}
                ...
                onClick={() =>
                  runAction(
                    "accept",
                    "Quote accepted. You can now create the order snapshot.",
          ...
          {quote?.status === "expired" ? (
            <div className={styles.expiredPanel} role="alert">
              <div>
                ...
                <p>
                  This quote can no longer be accepted. Request a fresh review
                  using the same current estimate.
                </p>
              </div>
              <button
                ...
                onClick={() =>
                  runAction(
                    "request-again",
                    "A fresh quote review was requested.",
                    () => client.requestQuote(design.id),
                  )
                }
                type="button"
              >
                {activeKey === "request-again"
                  ? "Requesting…"
                  : "Request a new quote"}
              </button>
            </div>
          ...
          {quote?.status === "accepted" && !order ? (
            <div className={styles.acceptedPanel}>
              <div>
                ...
                <p>
                  Create an immutable order snapshot. This records intent only
              ...
              <button
                ...
                onClick={() =>
                  runAction(
                    "order",
                    "Order snapshot created and design references locked.",
        ...
        <section className={styles.card} aria-labelledby="fulfillment-heading">
          <div className={styles.cardHeading}>
            <div>
              <p className={styles.eyebrow}>Steps 3–4</p>
              <h2 id="fulfillment-heading">Order and fulfillment</h2>
            </div>
            <span className={styles.stateBadge}>
              {order?.status.replace("-", " ") ?? "Awaiting order"}
            </span>
          </div>
          ...
          {order ? (
            <div className={styles.orderSummary}>
              ...
              <div>
                ...
                <strong>{formatMoney(order.acceptedTotal, locale)}</strong>
              </div>
            </div>
          ) : (
            <p className={styles.muted}>
              Fulfillment begins only after an issued quote is accepted and an
              order snapshot is created.
            </p>
          )}
              ...
              return (
                ...
                >
                  ...
                  <div>
                    ...
                    <p>{complete ? stage.detail : "Not started"}</p>
                  </div>
                  {current ? (
          ...
          {order && order.status !== "ready" ? (
            <p className={styles.hint}>
              Fulfillment updates are recorded by an authorized operator and
              will appear here.
            </p>
          ) : null}
        </section>
