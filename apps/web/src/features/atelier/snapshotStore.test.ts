...
import { getSnapshot, saveSnapshotRecord, SNAPSHOT_VERSION } from "./snapshotStore";

describe("saved piece snapshot storage", () => {
  it("reports session-only storage and retains an immutable Blob when IndexedDB is absent", async () => {
    ...
    const snapshot = { id, key: "piece-one", rendererVersion: SNAPSHOT_VERSION,
