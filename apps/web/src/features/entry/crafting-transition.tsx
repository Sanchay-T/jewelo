...
export function CraftingTransition({
  ...
  const [cancelledTaskId, setCancelledTaskId] = useState<string>();
  ...
  const rawCards = useMemo(
    () =>
      adaptPresentationCards(
        run,
        cancelledTaskId ? new Set([cancelledTaskId]) : new Set(),
      ),
    [cancelledTaskId, run],
  );
  const cards = useMemo(
  ...
  useEffect(() => {
    if (!run) return;
    setCancelledTaskId(
      window.sessionStorage.getItem(`caleums:crafting-cancelled:${run.id}`) ??
        undefined,
    );
  }, [run]);
  useEffect(() => {
        ...
        <section className="clm-generation-copy" aria-live="polite">
          ...
          {!ready && !failed && !cancelled && !blocked && !replay && (
            <button
              ...
              onClick={() => {
                ...
                if (active) {
                  setCancelledTaskId(active.id);
                  if (run)
                    window.sessionStorage.setItem(
                      `caleums:crafting-cancelled:${run.id}`,
                      active.id,
                    );
                  void client.cancelTask(design.id, active.id);
                }
              }}
            >
          ...
          {cancelled && (
            <button
              ...
              type="button"
              onClick={() => {
                setCancelledTaskId(undefined);
                void client.startRun(design.id);
              }}
            >
              Start again
            </button>
        ...
        <section className="clm-generation-stage">
          ...
          {ready && (
            <div className="clm-crafting-complete" role="status">
              ...
              <span>
                {replay ? "Opening Studio…" : "Every presentation is ready."}
              </span>
              <Link
                className="clm-primary"
