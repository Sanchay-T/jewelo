...
- Security fix 1 (implementer, web only) and tooling fix 1 (implementer) in flight on non-overlapping files. Pipeline fix 1 (presentation.ts, migrations, sharp bump) waits for fix pass 4 to release the file.
