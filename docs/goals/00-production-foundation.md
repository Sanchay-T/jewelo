...
A fresh checkout can install, lint, typecheck, test, build, run a minimal health surface, and exercise typed data/media/provider/workflow contracts through documented root commands. Cloud Run, Supabase, Trigger.dev, OpenAI, and fal development seams are configured without paid media calls. Authorized Cloud Run/Supabase/Trigger setup is verified or reduced to one precise external authorization.
...
- Add Cloud Run, Supabase, Trigger, OpenAI, and fal setup/runbooks and project commands.
...
- With authorization, inspect/deploy a Cloud Run staging revision, Supabase development/preview branch, and Trigger development/preview task; otherwise name the exact missing authorization.
