...
| Media UI | Embla Carousel, `react-zoom-pan-pinch`, `react-dropzone` | Direction filmstrip, real inspection, accessible upload. |
...
packages/
  ...
  ai/                   prompt compiler, provider ports, OpenAI adapters
...
OpenAI is not the Jewelo workflow engine. AI generation needs:
...
- one inspectable timeline across OpenAI and Supabase.

Trigger.dev owns orchestration. Supabase owns durable business truth. OpenAI executes model inference.

## Why direct OpenAI for stills

GPT Image 2 is locked as the image model, called directly so that OpenAI quota, usage and error handling stay under Jewelo's own control.
...
4 product stills concurrently
  each passes QA independently
    the studio still immediately releases its dependent stills
```
...
Git branch
  ...
  -> development OpenAI credentials
```
...
- OpenAI: provider ports and model profiles isolate vendor APIs.
