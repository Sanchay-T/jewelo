...
| Hosting | Google Cloud Run, Mumbai (`asia-south1`) | Buildpack-built immutable revisions; no long AI work in request handlers. |
...
| CI/CD | GitHub Actions + Cloud Build/Cloud Run + Supabase/Trigger preview integrations | Keyless OIDC deploys immutable images; each goal PR receives isolated preview seams and a proof packet. |
...
- Cloud Run: Next.js remains deployable to another Node-compatible host.
