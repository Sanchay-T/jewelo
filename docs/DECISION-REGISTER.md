...
Every finished studio still records the route on the asset as `verification_result.stillRoute`, and a stencil one records why as `verification_result.stillRouteReasons` (SP-2e2e): the closed snake_case codes the decision uses, never a letter of the customer's name, plus `free_route_off` when the switch is down and `release_names_stencil` when the pinned template names the stencil, so the reason why a still took the route it took is durable instead of having to be reconstructed from the rules. It is written to `assets.verification_result.stillRouteReasons` and read there with SQL: no operator surface shows it today. `GET /api/state` publishes an allowlist of four verification fields and not this one, and the operator review queue reads runs and tasks, never assets.
