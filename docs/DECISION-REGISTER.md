...
The second is still true and origami-ribbon Arabic is still a stencil; the first was a wording defect, not a letter defect, and UNIV-2 fixed the wording.
So `stillRoute` is an allowlist of what was photographed, and nothing else.
In Arabic that allowlist is now the standard Arabic base letters, U+0621-U+063A and U+0641-U+064A, in any position, in classical and diamond-rails (`ARABIC_FREE_LETTER_RANGES`, one constant so it can be narrowed again in one line).
It was six letters until 24 September 2026, when three labs measured the whole script instead of the two names SP-2d happened to draw.
UNIV-1 (`docs/goals/road-to-gold/lab-2026-09-24-universal/ledger.md`) put 16 common UAE Arabic names through the production free studio prompt, 16 in classical and 6 in diamond-rails, two takes each: a blind viewer read 48 of 48 correctly, so spelling does not depend on which letters the name carries.
One piece was the split: diamond-rails was one piece 12 of 12, classical 15 of 32, and the classical failures were almost all a dot or a three-dot cluster floating free of its letter.
UNIV-2 (`lab-2026-09-24-universal-2/ledger.md`) rewrote the Name paragraph's dot clause and scored the two candidate wordings blind in one session against the production wording, on the same 8 dotted names: the production wording 8 of 16 spelled and one piece, wording A 14 of 16, wording B 13 of 16.
Wording A is what the compiler emits on the free Arabic route now - every dot is a solid gold dot fused to its letter by a short straight bar as thick as the strokes, clusters fused to each other, and the two rings on short plain tabs that never curl into a stroke that could be read as a letter.
UNIV-3 (`lab-2026-09-24-universal-3/ledger.md`) drew the letters UNIV-1 never did - the hamza carriers أ إ آ ؤ, the emphatics ق ض ظ, and ك ج غ ذ ت and a final ى - on 12 names in diamond-rails, the construction UNIV-1 had already shown is one piece 12 of 12.
The ى-last and د-last position rules are gone with the six-letter table: they were written from two names, and UNIV-1's rails cells carried ا ر د و inside the word and came out one piece anyway.
What catches a dot that still floats is not a letter table, it is the readers and the three paid attempts per task.
On the 45 UNIV-1 stills they read, the piece reader made 0 false accepts and refused all three real diamond-rails split controls (`lab-2026-09-24-universal/reader-replay.md`); it is strict rather than lenient, refusing 16 stills the viewer measured as one piece, and a false refusal costs a retry inside the ceiling, not a wrong pendant.
That is the trade the widened set is taken on: a letter table narrow enough to be safe on its own would also send most real names to a stiffer photograph, and the reader plus the retries already fail closed in the direction that matters.
...
Framed-minimal Arabic returns to the free route only when a reader wording passes a fresh held-out set (SP-2a4); diamond-rails and classical Arabic held and stay free, and are the two constructions the widened letter set applies to.
In Latin the allowlist is the whole plain A-Z rather than only the letters that were photographed, and that lesser rigour is deliberate: the classical Latin face is one connected script whose failure modes are the two the rules already name, a floating tittle and a script capital that does not join what follows, and the four lab names (Muhammad, Omar, Love, Asma) drew only ten of the twenty-six lower case (u h a m d r o v e s) and four capitals (M O L A) between them, yet no failure appeared that was not one of those two, where Arabic failed on the letter itself.
The piece has to be stoneless and in the face the construction draws by default - the shopper picked no named face, so the specification carries `lettering: "classic"` and, in Arabic, `arabicStyle: "contemporary"` - and that is what the labs photographed, nothing else.
The face is the sharpest case, because the compiled words are byte for byte identical whichever face was chosen, so a Diwani order photographed free comes back in whatever the construction paragraph asks for; the stones sentence and the metal colour do reach the model as words (`{{stones_rule}}` is in the live packshot template and neither it nor the metal colour is route-aware), but no reader judges either, so both stay on what a lab drew.
Measured, and measured per script: yellow in both scripts, and rose in all four constructions but in Latin only - every rose cell is "Love" (`docs/goals/road-to-gold/lab-2026-09-24-free/ledger.md` lines 50, 56, 62 and 68, all pass; `lab-2026-09-24-free-route/ledger.md` line 22; the worn takes at `lab-2026-09-24-free-dependent/ledger.md` lines 54-55, where the rose was measured at hue 29-31 against 38-40 for yellow in the same scene).
So the proven list is per script (SP-2e2e): Latin yellow and rose, Arabic yellow only, and an Arabic name ordered in rose gold routes to the stencil (`metal_not_proven`) exactly as a white one does.
White gold was never drawn from a free prompt in either script, so it routes to the stencil (`metal_not_proven`), as does a specification with no `metalColor` at all.
A specification that carries the customer's own inspiration photograph also routes to the stencil (`inspiration_not_proven`): no lab drew a free piece with one, and the live minimal `image.packshot@v4` family writes an `Image N (role)` line for the stencil and the look only, so a free compile would hand the model an unlabelled customer photograph and no stencil to hold the letters.
Absence is not a default for the fields this section reads: the atelier writes `arabicStyle`, `lettering`, `stoneCoverage`, `gemstone` and `metalColor` on every new revision (`backendSpecification` in `apps/web/src/features/atelier/previewHandoff.ts`), so a specification missing one is a revision the route cannot read and it goes to the stencil (`piece_field_missing:<field>`).
...
A false `stencil` costs a slightly stiffer photograph; a false `free` costs a customer a pendant that spells their name in two pieces, or in a face they did not order.
...
Every finished studio still records the route on the asset as `verification_result.stillRoute`, and a stencil one records why as `verification_result.stillRouteReasons` (SP-2e2e): the closed snake_case codes the decision uses, never a letter of the customer's name, plus `free_route_off` when the switch is down and `release_names_stencil` when the pinned template names the stencil, so an operator reads the reason off the asset instead of guessing which rule fired.
A dependent view inherits its studio's route and records no reasons of its own.
...
The pairing is asserted before the reservation, in `apps/jobs/src/presentation.ts`, not only inside the compiler, and it is asserted against the words rather than against the route column: the compiler rewrites every `@role` tag into `Image N (role)` and writes the same label in the IMAGE ROLES header, so the stored `compiled_prompt` says "(stencil)" on every stencil compile, says it nowhere on a free one (`still_free_route_names_stencil` refuses the word) and says "(master)" on every dependent one.
Words that name a stencil with no signed stencil URL (`still_stencil_required`), words that name none while the request carries one (`still_free_route_carries_stencil`) and words that name a master with no studio photograph to copy (`still_master_required`) each stop the dispatch pre-spend with a stored code an operator can read.
