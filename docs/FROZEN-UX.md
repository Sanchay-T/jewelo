...
The screenshots in `docs/previews/` are visual references, not permission to copy bugs. Retain the premium cream/gold character while correcting inspection, progress, accessibility, and commercial trust.
...
Each representation can be `queued`, `generating`, `ready`, `retrying`, `failed`, `cancelled`, `unavailable`, or `available_on_request`. Placeholders are never selectable finished output.

```text
Direction 1  Product ready   Worn ready      Motion ready
Direction 2  Product ready   Worn generating Motion available on request
Direction 3  Product retry   Worn blocked     Motion blocked
Direction 4  Product ready   Worn ready       Motion available on request
```
...
- Product uses a square master; worn uses a 4:5 master without destructive cropping.
- Zoom supports pan, wheel, pointer, keyboard, and touch.
- Motion navigation and scrubbing use separate controls/gesture zones.
- Motion is generated for the selected or explicitly requested direction, not silently for all four.
