...
CREATE GENERATION RUN
  ├─ direction 1: product → worn → motion
  ├─ direction 2: product → worn → motion
  ├─ direction 3: product → worn → motion
  └─ direction 4: product → worn → motion
  ▼
INSPECT / COMPARE
...
Motion representation          9:16 primary, loopable preview
Final comparison               large inspection canvas; no forced 4:3 crop
