...
Browser
  ...
  v
Next.js on Google Cloud Run (Mumbai)
...
```text
development  -> persistent Supabase dev + Trigger DEV + optional local Next.js + dev provider keys
preview      -> Supabase PR branch + Trigger preview branch + Cloud Run staging + contract providers
staging      -> persistent Supabase staging + Trigger STAGING + bounded OpenAI/fal validation
production   -> Supabase Mumbai prod + Trigger PROD + Cloud Run production + approved provider quotas
```
